...
> **Recommended baseline:** pluripotent cells (e.g. iPSCs or ESCs). Because pluripotent cells express few lineage-specific markers, virtually any differentiated signature will appear as a positive LFC, maximising the diversity of detectable enriched cell types. Publicly available bulk pluripotent RNA-seq data can be used if an in-house baseline is unavailable, but samples should be processed together in the same experiment and library preparation to minimise batch effects.
